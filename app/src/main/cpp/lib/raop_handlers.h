...
static void
raop_handler_http_play(raop_conn_t *conn,
                     http_request_t *request, http_response_t *response,
                     char **response_data, int *response_datalen)
{
    const unsigned char *data;
    ...
    const char *contentLength  = http_request_get_header(request, "Content-Length");
    if(!contentType){
        logger_log(conn->raop->logger, LOGGER_DEBUG, "raop_handler_http_play contentType=%s",contentType);
    }
    if(!contentLength) {
        logger_log(conn->raop->logger, LOGGER_DEBUG, "raop_handler_http_play contentLength=%s",
                   contentLength);
    }
    data = (unsigned char *) http_request_get_data(request, &datalen);
    logger_log(conn->raop->logger, LOGGER_DEBUG, "raop_handler_http_play datalen=%d",datalen);
    http_response_add_header(response, "Content-Type", "application/octet-stream");

}

static void
raop_handler_airplay_pairverify(raop_conn_t *conn,
                        http_request_t *request, http_response_t *response,
                        char **response_data, int *response_datalen)
{
    unsigned char public_key[32];
