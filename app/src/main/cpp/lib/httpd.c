...
httpd_thread(void *arg)
{
	...
	while (1) {
		...
		if (httpd->open_connections < httpd->max_connections) {
            ...
            if (httpd->server_airplay_fd4 != -1) {
                FD_SET(httpd->server_airplay_fd4, &rfds);
                if (nfds <= httpd->server_airplay_fd4) {
                    nfds = httpd->server_airplay_fd4+1;
                }
            }
		}
		for (i=0; i<httpd->max_connections; i++) {
        ...
        if (httpd->open_connections < httpd->max_connections &&
            httpd->server_airplay_fd4 != -1 && FD_ISSET(httpd->server_airplay_fd4, &rfds)) {
            ret = httpd_accept_connection(httpd, httpd->server_airplay_fd4, 0);
            if (ret == -1) {
                break;
            } else if (ret == 0) {
                continue;
		...
		for (i=0; i<httpd->max_connections; i++) {
			...
			if (http_request_is_complete(connection->request)) {
				...
				// 回调收到的数据给raop
//                const char *cseq = http_request_get_header(connection->request, "CSeq");
//                if (!cseq){
//                    httpd->callbacks.http_conn_request(connection->user_data, connection->request, &response);
//                }else{
...
//                }
    ...
    if (httpd->server_airplay_fd4 != -1) {
        shutdown(httpd->server_airplay_fd4, SHUT_RDWR);
        closesocket(httpd->server_airplay_fd4);
        httpd->server_airplay_fd4 = -1;
    }
...
int
httpd_start(httpd_t *httpd, unsigned short *port,unsigned short *airplayPort)
{
   ...
   httpd->server_airplay_fd4 = netutils_init_socket(airplayPort, 0, 0);
    if (httpd->server_airplay_fd4 == -1) {
        logger_log(httpd->logger, LOGGER_ERR, "Error initialising socket %d", SOCKET_GET_ERROR());
        MUTEX_UNLOCK(httpd->run_mutex);
        return -1;
    ...
    if (httpd->server_airplay_fd4 != -1 && listen(httpd->server_airplay_fd4, backlog) == -1) {
        logger_log(httpd->logger, LOGGER_ERR, "Error listening to server_airplay_fd4 IPv4 socket");
        closesocket(httpd->server_fd4);
        closesocket(httpd->server_fd6);
        closesocket(httpd->server_airplay_fd4);
        MUTEX_UNLOCK(httpd->run_mutex);
