        ...
        *response = http_response_init("HTTP/1.1", 200, "OK");
        if (!strcmp(method, "POST") && !strcmp(url, "/pair-verify")){
            if (pairing_session_check_handshake_status(conn->pairing)) {
                raop_handler_airplay_pairverify(conn, request, *response, &response_data, &response_datalen);
            }
        }
    }
...
static void
http_conn_request(void *ptr, http_request_t *request, http_response_t **response)
{
    raop_conn_t *conn = ptr;
    logger_log(conn->raop->logger, LOGGER_DEBUG, "http_conn_request");
    const char *method;
    const char *url;
    const char *cseq;
    ...
    method = http_request_get_method(request);
    if (method){
        logger_log(conn->raop->logger, LOGGER_DEBUG, "http_conn_request method=%s",method);
    }
    url = http_request_get_url(request);
    if (url){
        logger_log(conn->raop->logger, LOGGER_DEBUG, "http_conn_request url=%s",url);
    }
    if (!method ) {
        return;
    }
...
//    logger_log(conn->raop->logger, LOGGER_DEBUG, "Handling request %s with URL %s", method, url);
    ...
    if (!strcmp(method, "GET") && !strcmp(url, "/info")) {
        ...
        handler = &raop_handler_http_play;
    }
    if (handler != NULL) {
        handler(conn, request, *response, &response_data, &response_datalen);
    }
...
raop_init(int max_clients, raop_callbacks_t *callbacks)
{
    ...
    httpd_cbs.http_conn_request = &http_conn_request;
...
void
raop_set_port(raop_t *raop, unsigned short port,unsigned short airplayPort)
{
    assert(raop);
    raop->port = port;
    raop->airplayPort = airplayPort;
}
...
unsigned short
raop_get_airplay_port(raop_t *raop)
{
    assert(raop);
    return raop->airplayPort;
}
...
int
raop_start(raop_t *raop, unsigned short *port,unsigned short *airplayPort)
{
	assert(raop);
	assert(port);
	return httpd_start(raop->httpd, port,airplayPort);
}
