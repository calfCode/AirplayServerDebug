...
void OpenSLRender::RenderAudioFrame(unsigned short *pData, int dataSize) {
    LOGD("OpenSLRender::RenderAudioFrame pData=%p, dataSize=%d", pData, dataSize);
...
void OpenSLRender::HandleAudioFrameQueue() {
    LOGD("OpenSLRender::HandleAudioFrameQueue QueueSize=%lu", m_AudioFrameQueue.size());
    ...
    std::unique_lock<std::mutex> lock(m_Mutex);
    AudioFrame *audioFrame = m_AudioFrameQueue.front();
    LOGD("OpenSLRender::HandleAudioFrameQueue bbbbbbbbbbbb");
    if (nullptr != audioFrame && m_AudioPlayerPlay) {
        LOGD("OpenSLRender::HandleAudioFrameQueue cccccccccccccccccc");
        SLresult result = (*m_BufferQueue)->Enqueue(m_BufferQueue, audioFrame->data, (SLuint32) audioFrame->dataSize);
        LOGD("OpenSLRender::HandleAudioFrameQueue result=%d", result);
        if (result == SL_RESULT_SUCCESS) {
            m_AudioFrameQueue.pop();
            delete audioFrame;
        }
